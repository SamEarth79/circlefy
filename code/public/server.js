(function () {
	...
	function updateFirst(event) {
		...
		let containerWidth = isMobile.matches ? "24" : "34";
		let containerHeight = isMobile.matches ? "24" : "34";
	...
	var artistsProfileSource = document.getElementById(
			"artists-profile-template"
		).innerHTML,
	...
	function logOut() {
		...
		window.location.href = "https://circlefy.onrender.com/";
	...
	function setCircleStyles(artistsProfile, response) {
		...
		const circle1_size = isMobile.matches ? 62 : 80;
		const circle1_radius = isMobile.matches ? "6em" : "8em";
		let circle1_rot = 290;
		const circle1_angle = 360 / circle1_count;

		const circle2_count = isMobile.matches ? 16 : 18;
		const circle2_size = isMobile.matches ? 52 : 70;
		...
		for (let i = 0; i < artistsProfile.children[0].children.length; i++) {
			if (
				artistsProfile.children[0].children[i].classList[0] ===
				"circle-1"
			) {
		...
		for (let i = 0; i < artistsProfile.children[0].children.length; i++) {
			if (
				artistsProfile.children[0].children[i].classList[0] === "tags-1"
			) {
		...
		tag1_rot = 290;
		tag_size = isMobile.matches ? "9" : "12";
		tag1_radius = isMobile.matches ? "10.5em" : "11em";
		...
		for (let i = 0; i < artistsProfile.children[0].children.length; i++) {
			if (
				artistsProfile.children[0].children[i].classList[0] === "tags-2"
			) {
		...
		tag2_rot = 100;
		tag2_radius = isMobile.matches ? "17.4em" : "17.5em";
	...
	function getArtists() {
		$.ajax({
			...
			success: function (response) {
				...
				artistsProfilePlaceholder.innerHTML =
					artistsProfileTemplate(allResponse);
				$("#login").hide();
				$("#loggedin").show();

				const artistsProfile =
					document.getElementById("artists-profile");
